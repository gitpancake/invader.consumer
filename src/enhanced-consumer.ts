...
import { ConcurrentProcessor, createImageProcessor } from "./util/concurrent/processor";
import { metricsCollector } from "./util/monitoring/metrics";
import { createHealthChecker, HealthChecker } from "./util/monitoring/health-checker";
...
class EnhancedFlashConsumer extends RabbitMQBaseConsumer {
  ...
  constructor() {
    ...
    console.log('[EnhancedFlashConsumer] Initializing with configuration:', configManager.getSummary());

    // Initialize components
    this.batchUpdater = new BatchUpdater(databasePool.getPool(), config.processing.batchSize);
    this.healthChecker = createHealthChecker(databasePool.getPool(), config.monitoring.interval);
    this.processingRateLimiter = new RateLimiter(config.processing.rateLimit);
    ...
    this.concurrentProcessor = createImageProcessor<Flash, void>({
      ...
      onProgress: (completed, total, active) => {
        ...
        if (completed % 50 === 0) { // Log progress every 50 items
          console.log(`[EnhancedFlashConsumer] Progress: ${completed}/${total} (${active} active)`);
        }
      },
      onError: (error, flash) => {
        console.error(`[EnhancedFlashConsumer] Processing failed for flash ${(flash as Flash)?.flash_id}:`, error.message);
      }
    });
  ...
  private setupCircuitBreakers(): void {
    ...
    circuitBreakerRegistry.create('image_download', this.downloadImage.bind(this), {
      failureThreshold: 10,
      resetTimeout: 60000,
      monitoringPeriod: 300000
    });

    circuitBreakerRegistry.create('ipfs_upload', this.uploadToIPFS.bind(this), {
      failureThreshold: 15,
      resetTimeout: 120000,
      monitoringPeriod: 300000
    });
  ...
  private async monitorBatchResults(): Promise<void> {
    try {
      ...
      console.error('[EnhancedFlashConsumer] Batch monitoring error:', error);
  ...
  private logSystemStatus(): void {
    ...
    console.log(`[EnhancedFlashConsumer] Status: Processing=${metrics.processingRate.toFixed(1)}/s, Memory=${memory.usage.toFixed(1)}%, Errors=${metrics.errorRate.toFixed(1)}%, Health=${health?.status || 'unknown'}, Circuits=${circuitBreakers.healthy}/${circuitBreakers.total}`);
  }
    ...
    try {
      ...
      if (ipfsHash) {
        ...
        if (flash.flash_id % 100 === 0) {
          console.log(`[EnhancedFlashConsumer] Successfully processed flash ${flash.flash_id}: ${ipfsHash}`);
        }
      }
      ...
      console.error(`[EnhancedFlashConsumer] Processing failed for flash ${flash.flash_id}: ${errorMsg}`);
      
      this.consecutiveApiFailures++;
      
      // Emergency shutdown on too many failures
      if (this.consecutiveApiFailures >= 50) {
        console.error(`[EnhancedFlashConsumer] CRITICAL: Too many consecutive failures (${this.consecutiveApiFailures}) - initiating shutdown`);
        process.exit(1);
      }
  ...
  private async isAlreadyProcessed(flashId: number): Promise<boolean> {
    try {
      ...
      console.error(`[EnhancedFlashConsumer] Database check failed for flash ${flashId}:`, error);
  ...
  private async uploadToIPFS(imageData: Buffer): Promise<string> {
    ...
    const file = new File([new Uint8Array(imageData)], filename, { type: "image/jpeg" });
    ...
    if (response.status === 429) {
      ...
      throw new Error(`Rate limited by Pinata API - retry after: ${retryAfter}s`);
    }
  ...
  private async handleBatchUpdateResult(result: BatchUpdateResult): Promise<void> {
    if (result.failed.length > 0) {
      metricsCollector.incrementCounter('batch.failed', result.failed.length);
      ...
      for (const failedUpdate of result.failed) {
        try {
          ...
          console.error(`[EnhancedFlashConsumer] Failed to requeue flash ${failedUpdate.flash_id}:`, error);
        }
      }
    }
    
    if (result.successful > 0) {
      metricsCollector.incrementCounter('batch.successful', result.successful);
    }
  }
  ...
  private async createFlashMessageFromUpdate(update: { flash_id: number; ipfs_cid: string }): Promise<Flash | null> {
    try {
      ...
      console.error(`[EnhancedFlashConsumer] Error creating flash message for ${update.flash_id}:`, error);
    }
    return null;
  }
  ...
  private async requeueMessage(flash: Flash): Promise<void> {
    ...
    console.log(`[EnhancedFlashConsumer] Requeuing flash ${flash.flash_id} for retry`);
  }
  ...
  public async shutdown(): Promise<void> {
    ...
    try {
      ...
      while (this.activeProcessing > 0) {
        console.log(`[EnhancedFlashConsumer] Waiting for ${this.activeProcessing} active processes to complete...`);
        await new Promise(resolve => setTimeout(resolve, 1000));
      }

      // Shutdown batch updater
      await this.batchUpdater.shutdown();
      console.log('[EnhancedFlashConsumer] Batch updater shutdown complete');

      // Shutdown database pool
      await databasePool.shutdown();
      console.log('[EnhancedFlashConsumer] Database pool shutdown complete');
      ...
      console.error('[EnhancedFlashConsumer] Error during shutdown:', error);
      throw error;
    }
  }
}
...
(async () => {
  ...
  console.log(`[EnhancedFlashConsumer] Starting in ${testMode ? 'TEST' : 'PRODUCTION'} mode`);
  console.log(`[EnhancedFlashConsumer] Configuration: ${JSON.stringify(configManager.getSummary())}`);
  
  // Initialize proxy system
  await proxyRotator.initialize();
  
  if (proxyRotator.getTotalProxyCount() > 0) {
    console.log(`[EnhancedFlashConsumer] Proxy system: ${proxyRotator.getProxyCount()}/${proxyRotator.getTotalProxyCount()} proxies active`);
  } else {
    console.log('[EnhancedFlashConsumer] No proxy configured - using direct connections');
  }
  ...
  process.on('unhandledRejection', (reason, promise) => {
    console.error('[EnhancedFlashConsumer] Unhandled rejection at:', promise, 'reason:', reason);
    process.exit(1);
  });
