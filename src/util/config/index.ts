...
class ConfigManager {
    ...
    private loadConfiguration(): AppConfig {
        const rawConfig = {
            ...
            processing: {
                concurrency: this.parseNumber(
                    process.env.CONSUMER_CONCURRENCY,
                    3,
                ),
                batchSize: this.parseNumber(process.env.BATCH_SIZE, 100),
                rateLimit: this.parseNumber(
                    process.env.CONSUMER_RATE_LIMIT,
                    250,
                ),
            ...
            monitoring: {
                ...
                interval: this.parseNumber(
                    process.env.MONITORING_INTERVAL,
                    30000,
                ),
                retention: this.parseNumber(
                    process.env.MONITORING_RETENTION,
                    86400000,
                ),
                alerts: {
                    memory: {
                        warning: this.parseFloat(
                            process.env.MEMORY_WARNING_THRESHOLD,
                            0.8,
                        ),
                        critical: this.parseFloat(
                            process.env.MEMORY_CRITICAL_THRESHOLD,
                            0.9,
                        ),
                    },
                    errorRate: {
                        warning: this.parseNumber(
                            process.env.ERROR_RATE_WARNING,
                            5,
                        ),
                        critical: this.parseNumber(
                            process.env.ERROR_RATE_CRITICAL,
                            10,
                        ),
                    },
                    responseTime: {
                        warning: this.parseNumber(
                            process.env.RESPONSE_TIME_WARNING,
                            15000,
                        ),
                        critical: this.parseNumber(
                            process.env.RESPONSE_TIME_CRITICAL,
                            30000,
                        ),
                    },
                },
            ...
            proxy: {
                ...
                healthCheck: this.parseBoolean(
                    process.env.PROXY_HEALTH_CHECK,
                    true,
                ),
            ...
            pinata: {
                ...
                retryDelay: this.parseNumber(
                    process.env.PINATA_RETRY_DELAY,
                    2000,
                ),
            },
        };
    ...
    override(path: string, value: any): void {
        ...
        console.log(
            `[ConfigManager] Configuration override: ${path} = ${value}`,
        );
    }
    /**
     * Clear all overrides
    ...
    reload(): void {
        console.log(
            "[ConfigManager] Reloading configuration from environment...",
        );
        this.config = this.loadConfiguration();
        this.validateConfiguration();
    }
...
export function validateEnvironment(): void {
    const required = ["DATABASE_URL", "RABBITMQ_URL", "RABBITMQ_QUEUE"];
    const missing = required.filter((key) => !process.env[key]);
    if (missing.length > 0) {
        console.error(
            "[ConfigManager] Missing required environment variables:",
            missing,
        );
        throw new Error(
    ...
    // Environment validated - shown in startup banner
}
