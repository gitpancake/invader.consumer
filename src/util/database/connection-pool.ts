...
export class DatabaseConnectionPool extends EventEmitter {
    ...
    private createPool(): Pool {
        ...
        // Pool config shown in startup banner
    ...
    private setupEventListeners(): void {
        this.pool.on("connect", (client: PoolClient) => {
            ...
            // Connection logging suppressed - use stats for monitoring
        ...
        this.pool.on("remove", (client: PoolClient) => {
            ...
            console.log(
                `[DatabaseConnectionPool] Connection removed (total: ${this.pool.totalCount})`,
            );
    ...
    private startMonitoring(): void {
        ...
        if (monitoringConfig.enabled) {
            ...
            // Pool monitoring started silently
        }
    }
    ...
    private recordMetrics(): void {
        ...
        const utilization =
            this.stats.totalCount > 0
                ? ((this.stats.totalCount - this.stats.idleCount) /
                      this.stats.totalCount) *
                  100
                : 0;
        ...
        for (let attempt = 1; attempt <= dbConfig.retries.max + 1; attempt++) {
            try {
                ...
                if (configManager.isDevelopment() && queryTime > 5000) {
                    console.warn(
                        `[DatabaseConnectionPool] Slow query (${queryTime}ms):`,
                ...
                if (attempt <= dbConfig.retries.max && isRetryableError) {
                    const delay =
                        dbConfig.retries.delay * Math.pow(2, attempt - 1);
                    console.warn(
                        `[DatabaseConnectionPool] Query failed, retrying in ${delay}ms (attempt ${attempt}/${dbConfig.retries.max + 1}):`,
                        lastError.message,
                    );
                ...
                console.error(
                    `[DatabaseConnectionPool] Query failed after ${attempt} attempts:`,
                    lastError.message,
                );
                throw lastError;
            }
    ...
    async getClient(): Promise<PoolClient> {
        ...
        try {
            ...
            if (waitTime > 5000) {
                console.warn(
                    `[DatabaseConnectionPool] Client acquisition took ${waitTime}ms`,
                );
            }

            metricsCollector.recordMetric(
                "database.client.acquisition_time",
                waitTime,
            );
            return client;
        } catch (error) {
            console.error(
                "[DatabaseConnectionPool] Failed to acquire client:",
                error,
            );
            metricsCollector.incrementCounter(
                "database.client.acquisition_errors",
            );
            throw error;
        }
    }
    ...
    async transaction<T>(
        ...
        try {
            ...
            console.error(
                "[DatabaseConnectionPool] Transaction failed:",
                error,
            );
            throw error;
        } finally {
    ...
    async healthCheck(): Promise<{
        healthy: boolean;
        details: PoolStats & { responseTime: number };
    }> {
        const startTime = Date.now();
    ...
    async shutdown(): Promise<void> {
        ...
        console.log(
            "[DatabaseConnectionPool] Shutting down connection pool...",
        );
        ...
        try {
            ...
            console.error(
                "[DatabaseConnectionPool] Error during shutdown:",
                error,
            );
        }
