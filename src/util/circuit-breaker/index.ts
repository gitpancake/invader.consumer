...
export class CircuitBreaker<T> extends EventEmitter {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      if (this.state === CircuitBreakerState.OPEN) {
        if (this.canAttemptReset()) {
          ...
          const error = new Error(`Circuit breaker is OPEN. Next retry: ${new Date(this.nextRetryTime!)}`);
  ...
  private onSuccess(): void {
    ...
    if (this.state === CircuitBreakerState.HALF_OPEN) {
      ...
      this.failureCount = 0;
      console.log(`[CircuitBreaker:${this.name}] State changed to CLOSED after successful half-open request`);
      this.emit('stateChange', this.state);
  ...
  private onFailure(): void {
    ...
    if (this.shouldOpenCircuit()) {
      ...
      console.warn(`[CircuitBreaker:${this.name}] Circuit OPENED due to ${this.failureCount} failures`);
      this.emit('stateChange', this.state);
      ...
      this.state = CircuitBreakerState.OPEN;
      this.nextRetryTime = Date.now() + this.options.resetTimeout;
      console.warn(`[CircuitBreaker:${this.name}] Returned to OPEN state after half-open failure`);
      this.emit('stateChange', this.state);
    }
  }
  ...
  private shouldOpenCircuit(): boolean {
    ...
    // Check failure rate
    const currentFailureRate = this.requestCount > 0 ? this.failureCount / this.requestCount : 0;
  ...
  private resetCounters(): void {
    if (this.state === CircuitBreakerState.CLOSED) {
      ...
      if (oldRequestCount > 0) {
        console.log(`[CircuitBreaker:${this.name}] Reset counters - Previous period: ${oldRequestCount} requests, ${oldFailureCount} failures`);
      }
    }
  }
  ...
  private recordMetrics(success: boolean, duration: number): void {
    metricsCollector.recordMetric(`circuit_breaker.${this.name}.duration`, duration);
    metricsCollector.incrementCounter(`circuit_breaker.${this.name}.requests`);
    
    if (success) {
      metricsCollector.incrementCounter(`circuit_breaker.${this.name}.success`);
    } else {
      metricsCollector.incrementCounter(`circuit_breaker.${this.name}.failures`);
    }
    ...
    metricsCollector.recordMetric(`circuit_breaker.${this.name}.state`, stateValue);
  }
  ...
  reset(): void {
    ...
    console.log(`[CircuitBreaker:${this.name}] Manual reset - state: CLOSED`);
    this.emit('stateChange', this.state);
  }
...
export class CircuitBreakerRegistry {
  ...
  resetAll(): void {
    this.breakers.forEach((breaker, name) => {
      breaker.reset();
      console.log(`[CircuitBreakerRegistry] Reset circuit breaker: ${name}`);
    });
  }
  ...
  remove(name: string): boolean {
    ...
    if (removed) {
      console.log(`[CircuitBreakerRegistry] Removed circuit breaker: ${name}`);
    }
    return removed;
  }
