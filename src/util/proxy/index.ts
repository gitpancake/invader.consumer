...
export class ProxyRotator {
    ...
    private proxyHealthStatus: Map<
        string,
        { working: boolean; lastChecked: number; consecutiveFailures: number }
    > = new Map();
    ...
    public async initialize(): Promise<void> {
        if (this.proxies.length > 0 && !this.healthCheckInProgress) {
            ...
            if (this.isOxylabs) {
                // Premium proxy service detected - skip health checks (they handle failures internally)
    ...
    private loadProxiesFromEnv(): void {
        ...
        if (proxyList) {
            try {
                ...
                // Proxies loaded silently - count shown in startup banner
                ...
                if (this.isOxylabs) {
                    if (
                        this.proxies.some((p) => p.host.includes("oxylabs.io"))
                    ) {
                        // Oxylabs premium proxy detected
                    } else if (
                        this.proxies.some((p) =>
                            p.host.includes("dataimpulse.com"),
                        )
                    ) {
                        // DataImpulse premium proxy detected
                    }
                }
            } catch (error) {
            ...
            // No proxy configured - shown in startup banner
        }
    }
    ...
    private async performInitialHealthCheck(): Promise<void> {
        ...
        console.log(
            `[ProxyRotator] 🔍 Starting health check for ${this.proxies.length} proxies...`,
        );
        ...
        console.log(
            `[ProxyRotator] 📊 Health check complete: ${this.workingProxies.length}/${this.proxies.length} proxies working`,
        );
    ...
    private async checkProxyHealth(proxy: ProxyConfig): Promise<boolean> {
        try {
            ...
            console.log(
                `[ProxyRotator] 🔍 Testing proxy URL: ${proxyUrl.replace(/:([^:@]+)@/, ":***@")}`,
            ...
            console.log(
                `[ProxyRotator] 🔍 Proxy ${proxy.host}:${proxy.port} health check: status ${response.status} = ${isHealthy ? "HEALTHY" : "UNHEALTHY"}`,
            );
            ...
            console.log(
                `[ProxyRotator] 🔍 Proxy ${proxy.host}:${proxy.port} health check ERROR: ${error.message} (407: ${is407Error})`,
            );

            if (is407Error) {
                console.log(
                    `[ProxyRotator] 🚫 Proxy ${proxy.host}:${proxy.port} - 407 Authentication failure (credentials may be incorrect)`,
                );
                return false;
            }
    ...
    private async recheckFailedProxies(): Promise<void> {
        ...
        console.log(
            `[ProxyRotator] 🔄 Re-checking ${failedProxies.length} failed proxies...`,
        );
        ...
        console.log(
            `[ProxyRotator] 📊 Recheck complete: ${this.workingProxies.length}/${this.proxies.length} proxies working`,
        );
    }
    ...
    private getNextProxy(): ProxyConfig | null {
        ...
        if (this.workingProxies.length > 0) {
            const proxy =
                this.workingProxies[
                    this.currentProxyIndex % this.workingProxies.length
                ];
            this.currentProxyIndex =
                (this.currentProxyIndex + 1) % this.workingProxies.length;
            return proxy;
        }
        ...
        console.log(
            "[ProxyRotator] ⚠️ No verified working proxies available, falling back to original pool",
        );
        ...
        while (attempts < maxAttempts) {
            ...
            if (failureCount < 3) {
                // Allow up to 3 failures before skipping
                return proxy;
            }
        ...
        if (attempts >= maxAttempts) {
            console.log(
                "[ProxyRotator] All proxies have failed multiple times, resetting failure counts",
            );
    ...
    private markProxyAsFailed(proxy: ProxyConfig, error?: Error): void {
        ...
        if (is407Error) {
            ...
            console.log(
                `[ProxyRotator] 🚫 Proxy ${proxyKey} BLACKLISTED - 407 Authentication failure`,
            );
        } else {
            ...
            this.failedProxies.add(proxyKey);
            console.log(
                `[ProxyRotator] Proxy ${proxyKey} failed (${currentCount + 1} times)`,
            );
        }
    }

    public createProxyAgent(targetUrl: string): {
        agent: any;
