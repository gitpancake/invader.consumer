...
export class MemoryOptimizer extends EventEmitter {
    ...
    startMonitoring(intervalMs: number = 30000): void {
        ...
        this.monitoringInterval = setInterval(() => {
            ...
            if (stats.usage > this.gcThreshold) {
                console.log(
                    `[MemoryOptimizer] Memory usage at ${(stats.usage * 100).toFixed(1)}% - triggering GC`,
                );
        ...
        // Memory monitoring started - config shown in startup banner
    }
    ...
    async processBatches<T, R>(
        ...
        console.log(
            `[MemoryOptimizer] Processing ${total} items in batches of ${batchSize} (concurrency: ${maxConcurrency})`,
        );
        ...
        for (let i = 0; i < items.length; i += batchSize * maxConcurrency) {
            ...
            const batchPromises = batches.map(async (batch) => {
                try {
                    ...
                    console.error(
                        `[MemoryOptimizer] Batch processing error:`,
                        error,
                    );
                    return [];
                }
            ...
            if (stats.usage > gcThreshold) {
                console.log(
                    `[MemoryOptimizer] Memory usage at ${(stats.usage * 100).toFixed(1)}% - cleaning up`,
                );
            ...
            if (progress % 10 === 0 || processed === total) {
                console.log(
                    `[MemoryOptimizer] Progress: ${processed}/${total} (${progress.toFixed(1)}%) - Memory: ${stats.heapUsed}MB`,
                );
            }
        }

        console.log(
            `[MemoryOptimizer] Batch processing complete: ${results.length} results`,
        );
        return results;
    }
    ...
    async processStream<T>(
        ...
        console.log(
            `[MemoryOptimizer] Stream processing complete: ${processed} items processed`,
        );
    }
    ...
    forceGarbageCollection(): void {
        if (global.gc) {
            ...
            const stats = this.getMemoryStats();
            console.log(
                `[MemoryOptimizer] GC completed - Memory: ${stats.heapUsed}MB (${(stats.usage * 100).toFixed(1)}%)`,
            );
        } else {
            console.log(
                "[MemoryOptimizer] GC not available - start with --expose-gc flag",
            );
        }
    }
