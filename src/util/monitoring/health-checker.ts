...
export class HealthChecker extends EventEmitter {
  ...
  start(): void {
    ...
    console.log(`[HealthChecker] Started health monitoring (interval: ${this.checkIntervalMs}ms)`);
  }
  ...
  async runHealthCheck(): Promise<HealthStatus> {
    ...
    try {
      ...
      const [database, pinata, memory, disk, processing] = await Promise.allSettled([
        this.checkDatabase(),
        this.checkPinata(),
        this.checkMemory(),
        this.checkDisk(),
      ...
      if (overallStatus !== 'healthy') {
        console.warn(`[HealthChecker] System status: ${overallStatus.toUpperCase()}`);
        this.logFailingChecks(checks);
      }

      // Record metrics
      metricsCollector.recordMetric('health.check_duration', Date.now() - startTime);
      metricsCollector.recordMetric('health.passing_checks', summary.passing);
      metricsCollector.recordMetric('health.failing_checks', summary.failing);
      ...
      const failedStatus: HealthStatus = {
        ...
        uptime: Math.floor(process.uptime()),
        checks: {
          database: { status: 'fail', message: 'Health check failed', duration: 0 },
          pinata: { status: 'fail', message: 'Health check failed', duration: 0 },
          memory: { status: 'fail', message: 'Health check failed', duration: 0 },
          disk: { status: 'fail', message: 'Health check failed', duration: 0 },
          processing: { status: 'fail', message: 'Health check failed', duration: 0 }
        },
  ...
  private logFailingChecks(checks: HealthStatus['checks']): void {
    Object.entries(checks).forEach(([name, result]) => {
      if (result.status !== 'pass') {
        console.warn(`[HealthChecker] ${name}: ${result.status.toUpperCase()} - ${result.message}`);
      }
    });
  }
...
// Factory function
export function createHealthChecker(dbPool: any, intervalMs?: number): HealthChecker {
