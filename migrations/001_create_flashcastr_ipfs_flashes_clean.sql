...
CREATE TABLE flashcastr_ipfs_flashes (
    ...
    CONSTRAINT fk_flashcastr_ipfs_flash_id FOREIGN KEY (flash_id) REFERENCES flashes(flash_id),
    
    -- Ensure one record per flash
    CONSTRAINT unique_flashcastr_ipfs_flash_id UNIQUE (flash_id)
);

-- Create indexes for faster lookups
CREATE INDEX idx_flashcastr_ipfs_flash_id ON flashcastr_ipfs_flashes(flash_id);
CREATE INDEX idx_flashcastr_ipfs_cid ON flashcastr_ipfs_flashes(ipfs_cid);
